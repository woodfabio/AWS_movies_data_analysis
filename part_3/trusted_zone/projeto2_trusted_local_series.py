...
eric_roberts_series = spark_df.filter(spark_df['artist_name'] == 'Eric Roberts')
...
eric_roberts_series = eric_roberts_series.withColumn("genre", explode(split("genre", ",")))

# split the "profession" column and explode the resulting array into separate rows
eric_roberts_series = eric_roberts_series.withColumn("profession", explode(split("profession", ",")))

print("="*50)
print("="*50)
eric_roberts_series.show()
print("="*50)
print("="*50)
                
# --------------------------------------------------------------------------
# repartition the DataFrame to a single partition
eric_roberts_series = eric_roberts_series.repartition(1)

# --------------------------------------------------------------------------
# turn DataFrame into glue DynamicFrame
new_dynamic_frame = DynamicFrame.fromDF(eric_roberts_series, glueContext, "new_dynamic_frame")
...
sink.setCatalogInfo(catalogDatabase="dl_trusted_zone", catalogTableName="trusted_local_series")
